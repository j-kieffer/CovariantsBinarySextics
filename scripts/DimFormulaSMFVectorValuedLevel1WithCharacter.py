...
def dimension_cusp_forms_SP4Z(J, k):
    ...
    res += QQ(-1)**k * (QQ(2)**(-7) * QQ(3)**(-2) * QQ(7) * (k-2) * (2*j+k-1) - QQ(2)**(-4) * QQ(3)**(-1) * (2*j+2*k-3) + QQ(2)**(-5) * QQ(3))
    res += QQ(-1)**j * (QQ(2)**(-7) * QQ(3)**(-1) * QQ(5) * (2*j+2*k-3) - QQ(2)**(-3))
    res += QQ(-1)**k * QQ(-1)**j * QQ(2)**(-7) * (2*j+1)
    ...
    res += (QQ(-1)**k * i**j * QQ(2)**(-5) * (i+1)).trace()
    res += (i**k * QQ(-1)**j * (QQ(2)**(-6) * QQ(3)**(-1) * (k-2) - QQ(2)**(-4))).trace()
    ...
    res += ((sigma**7)**k * QQ(-1)**j * QQ(2)**(-3) * QQ(3)**(-2) * (sigma**2+1)).trace()
...
def dimension_V(j, k):
    if (k % 2) == 0:
        d1 = QQ(2)**(-1) * dimension_cusp_forms(1, k+j/2)
        d2 = QQ(2)**(-1) * sum([dimension_cusp_forms(1, k+j-2*a) * dimension_cusp_forms(1, k+2*a) for a in range(j/2+1)])

    if (k % 2) == 1:
        d1 = - QQ(2)**(-1) * dimension_cusp_forms(1, k+j/2)
        d2 = QQ(2)**(-1) * sum([dimension_cusp_forms(1, k-1+j-2*a) * dimension_cusp_forms(1, k+1+2*a) for a in range(j/2)])
